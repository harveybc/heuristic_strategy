...
def next(self):
    ...
    if self.position:
        ...
        if self.current_direction == 'long':
            ...
            predicted_min = self.get_predicted_min(dt_hour, current_price)
            print(f"[DEBUG] (Long) Predicted_min: {predicted_min:.5f}")

            # Exit conditions
            if current_price >= self.current_tp:
                print(f"[DEBUG] (Long) Exit condition met: TP reached at {current_price:.5f}")
                self.close()
                return
            if predicted_min < self.current_sl:
                print(f"[DEBUG] (Long) Exit condition met: SL breached at {predicted_min:.5f}")
                self.close()
                return
            ...
            predicted_max = self.get_predicted_max(dt_hour, current_price)
            print(f"[DEBUG] (Short) Predicted_max: {predicted_max:.5f}")

            # Exit conditions
            if current_price <= self.current_tp:
                print(f"[DEBUG] (Short) Exit condition met: TP reached at {current_price:.5f}")
                self.close()
                return
            if predicted_max > self.current_sl:
                print(f"[DEBUG] (Short) Exit condition met: SL breached at {predicted_max:.5f}")
                self.close()
                return

        return  # Do not attempt new entries if still in a position.

    # Reset trade low and high if not in position
    self.trade_low = current_price
    self.trade_high = current_price
    ...
    daily_preds = [row[f'Prediction_d_{i}'] for i in range(1, self.num_daily_preds + 1) if f'Prediction_d_{i}' in row]
    ...
    print(f"[DEBUG] Entry conditions - Long RR: {rr_buy:.2f}, Short RR: {rr_sell:.2f}")
...
def compute_size(self, rr):
    ...
    return max(0, min(size, max_from_cash))  # Ensure size is non-negative
